...
class VideoModel: NSObject {
    private let maxRemoteVideoTileCount = 1000
...
//        logger.info(msg: "updateRemoteVideoStatesBasedOnActiveSpeakers \(activeSpeakerIds)")
...
//        }
    ...
    func getRemoteVideoTileState(_ attendeeId: String) -> VideoTileState? {
        if let index = remoteVideoTileStates.firstIndex(where: { $0.1.attendeeId == attendeeId }) {
            return remoteVideoTileStates[index].1
        }
        return nil
    }
    
    func getLocalVideoTileState() -> VideoTileState? {
