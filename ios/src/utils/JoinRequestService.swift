...
class JoinRequestService: NSObject {
    ...
    static func getSessionConfig(json: [String:Any], completion: @escaping (MeetingSessionConfiguration?) -> Void) {
        do {
            let dataJson = try JSONSerialization.data(withJSONObject: json, options: [])
            guard let meetingSessionConfiguration = self.processJson(data: dataJson) else {
                completion(nil)
                return
            }
